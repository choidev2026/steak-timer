...
class TimerReducerTest {
    ...
    fun `Idle은 Start 외 입력을 무시한다`() {
        assertEquals(Idle, Idle.reduce(TimerIntent.Tap))
        assertEquals(Idle, Idle.reduce(TimerIntent.LongPress))
    ...
    @Test
    fun `Alerting에서 Tap하면 다음 인터벌 Running + cycle+1`() {
        val alerting = Alerting(INTERVAL, cycle = 2)
        val next = alerting.reduce(TimerIntent.Tap)
        assertEquals(Running(INTERVAL, INTERVAL, cycle = 3), next)
    }

    @Test
    fun `Running에서 Tap하면 조기 뒤집기 재시작 + cycle+1`() {
        val running = Running(INTERVAL, 42_000, cycle = 1)
        val next = running.reduce(TimerIntent.Tap)
        assertEquals(Running(INTERVAL, INTERVAL, cycle = 2), next)
    }

    @Test
    fun `Running에서 LongPress하면 ConfirmStop(resumeTo 보존)`() {
        val running = Running(INTERVAL, 42_000, cycle = 1)
        val next = running.reduce(TimerIntent.LongPress)
        assertEquals(ConfirmStop(resumeTo = running), next)
    }

    @Test
    fun `Alerting에서는 LongPress를 무시한다`() {
        val alerting = Alerting(INTERVAL, cycle = 0)
        assertEquals(alerting, alerting.reduce(TimerIntent.LongPress))
    }
    ...
    fun `핵심 루프 - 시작 후 완주 뒤집기 조기 뒤집기가 cycle을 누적`() {
        ...
        s = s.reduce(TimerIntent.Tap)                       // 정상 뒤집기 -> Running cycle 1
        s = s.reduce(TimerIntent.Tap)                       // 조기 뒤집기 -> Running cycle 2
